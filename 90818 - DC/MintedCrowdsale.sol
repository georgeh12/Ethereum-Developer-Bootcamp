...
contract UniCoinICO{
    ...
    */
    ...
    constructor(uint256 _tokensPerWei, address _wallet, UniCoin _token) public payable {
    ...
    function buyTokens(address _to) public payable {
        ...
        _processPurchase(_to, newTokens);
        
        // Raise the event associated with a token purchase.
        emit TokenPurchase(from, _to, paid, newTokens);
    ...
    function _preValidatePurchase(address _to, uint256 _amount) pure internal {
    ...
    function _getTokenAmount(uint256 paid) view internal returns (uint256) {
        return paid.mul(tokensPerWei);
    }
